...
def main() -> None:
    ...
    pipeline_changed = any(f.startswith("pipeline/") for f in changed_files)
    frontend_changed = any(f.startswith("frontend/") for f in changed_files)

    # Pipeline Checks (Python data pipeline)
    pipeline_path = "pipeline"
    if os.path.exists(pipeline_path) and pipeline_changed:
        checks = [
            ("uv run ruff check src/ tests/", "Pipeline Lint (Ruff)"),
            ("uv run mypy src/", "Pipeline Type Check (MyPy)"),
            ("uv run pytest tests/ -v --cov", "Pipeline Tests"),
        ]

        for cmd, desc in checks:
            success, msg = run_check(cmd, pipeline_path, desc)
            if not success:
                errors.append(msg)
